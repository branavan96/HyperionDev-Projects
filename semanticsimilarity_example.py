...
ComplaintSimilarityNumbers = []
complaintsum = 0
for token in complaints:
    token = nlp(token)
    for token_ in complaints:
        token_ = nlp(token_)
        complaintsum += token.similarity(token_)
        ComplaintSimilarityNumbers.append(token.similarity(token_))
...
recipesum = 0
RecipeSimilarityNumbers =[]
for token in recipes:
    token = nlp(token)
    for token_ in recipes:
        token_ = nlp(token_)
        recipesum += token.similarity(token_)
        RecipeSimilarityNumbers.append(token.similarity(token_))
        # print(token.similarity(token_))
...
print("-------------Recipes similarity---------------")
RecipesComplaintssum = 0
RecipesVsComplaintsSimilarityNumbers = []
for token in recipes:
    token = nlp(token)
    for token_ in complaints:
        token_ = nlp(token_)
        RecipesComplaintssum += token.similarity(token_)
        RecipesVsComplaintsSimilarityNumbers.append(token.similarity(token_))
        # print(token.similarity(token_))
...
# The results for the average and standard deviation show that the recipes were more similar to each other than the complaints were similar to each other. 
# There was a larger range of similarity scores in complaints vs complaints than recipes vs recipes. 
# Complaints vs recipes has much lower average and much higher standard deviation.
# The model is quite small as there are only 6 items being compared. A larger model would give a clearer picture.
